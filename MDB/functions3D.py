...
from MDB.imresize3D import imresize, imresize_in, imresize_to_shape
...
def generate_noise(size,num_samp=1,device='cuda',type='gaussian', scale=1):
    if type == 'gaussian':
        ...
        # noise_ = np.random.normal(loc=0.0, scale=1,
        #                           size=(round(size[1] / scale), round(size[2] / scale), round(size[3] / scale)))
        # x = np.stack((noise_,) * 3, axis=-1)
        # x = x[:, :, :, :, None]
        # x = x.transpose((4, 3, 0, 1, 2))
        # x = torch.from_numpy(x)
        # x = move_to_gpu(x)
        # noise = x.type(torch.cuda.FloatTensor)
        # noise = upsampling(noise, size[1], size[2], size[3])
...
def read_image(opt):
    ...
    # x = np2torch(x,opt)
    # x = x[:,0:3,:,:]
    x = tifffile.imread('%s' %(opt.input_name))  # (张数，高，宽)
    # print(x.shape)
    x = np2torch(x, opt)
    x = x[:, 0:3, :, :, :]
    return x
...
#     x = tifffile.imread('%s' % (opt.input_name))
#     # print(x.shape)
#     x = np2torch(x, opt)
#     x = x[:, 0:3, :, :, :]
#     return x
...
def adjust_scales2image(real_, opt):
    # opt.scale1 = opt.max_size / max([real_.shape[2],real_.shape[3],real_.shape[4]])
