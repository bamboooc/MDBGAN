...
def train(opt):
    ...
    for scale_num in range(opt.stop_scale+1):
        ...
        img_real_save = functions.convert_image_np(reals[scale_num])  # 数据变为0-1
        tifffile.imsave('%s/real_scale1.tiff' % (opt.outf), img_real_save[:, :, :, 0].astype(np.float32))
        # print(img_real_save.shape)
        # print(img_real_save)
        tifffile.imsave('%s/real_scale.tiff' % (opt.outf), functions.convert_image_np(reals[scale_num])[:,:,:,2])
        # print(bbbb)
...
def train_single_scale(netD, netG, reals, fixed_noise, noise_amp, opt, depth, writer):
    ...
    for iter in _iter:
        ...
        if iter % 500 == 0 or iter+1 == opt.niter:
            ...
            # tifffile.imsave('{}/reconstruction_{}.tif'.format(opt.outf, iter+1), functions.convert_image_np(rec.detach())[:,:,:,2])
            # tifffile.imsave('{}/fake_sample_{}.tif'.format(opt.outf, iter+1), functions.convert_image_np(fake.detach())[:,:,:,2])
            # tifffile.imsave('%s/real_scale.tiff' % (opt.outf), functions.convert_image_np(reals[scale_num]))
            img_noise1 = functions.convert_image_np(fake.detach())
            img_noise2 = functions.convert_image_np(rec.detach())
            tifffile.imsave('{}/fake_sample_fake_{}.tiff'.format(opt.outf, iter+1), img_noise1[:, :, :, 0].astype(np.float32))
            tifffile.imsave('{}/fake_sample_rec_{}.tiff'.format(opt.outf, iter+1), img_noise2[:, :, :, 0].astype(np.float32))
...
def generate_samples(netG, opt, depth, noise_amp, writer, reals, iter, n=25):
    ...
    with torch.no_grad():
        for idx in range(n):
            ...
            # tifffile.imsave('{}/gen_sample_{}.tif'.format(dir2save, idx), sample.detach())
