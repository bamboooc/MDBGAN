...
from MDB.imresize3D import imresize, imresize_to_shape
from MDB import rbfl3d
...
class ContentBranch(nn.Module):
    def __init__(self, channel):
        ...
        self.fc = nn.Sequential(nn.Conv3d(channel, channel // 16, 1, bias=False),
                                nn.ReLU(),
                                nn.Conv3d(channel // 16, channel, 1, bias=False))
...
class LayoutBranch(nn.Module):
    def __init__(self, kernel_size=1):
        ...
        self.conv1 = nn.Conv3d(64, 1, kernel_size, padding=kernel_size // 2, bias=False)
        self.sigmoid = nn.Sigmoid()
...
class Generator3DBlock(nn.Module):
    def __init__(self, in_channel, out_channel,stride=1,bn=True):
        super(Generator3DBlock, self).__init__()
        # self.head = ConvBlock(in_channel,out_channel,ker_size=1,padding=0,stride=stride,bn=bn)
        out_channel = out_channel//2
        self.branch0 = nn.Sequential(
            ConvBlock(in_channel, out_channel, ker_size=3, padding=1, stride=stride,bn=bn)
        )
        self.branch1 = nn.Sequential(
            # ConvBlock(in_channel, out_channel, ker_size=3, padding=1, stride=stride, dilation=1,bn=bn),
            ConvBlock(out_channel, out_channel, ker_size=3, padding=1, stride=stride)
        )
        ...
        self.liner = ConvBlock(out_channel*3, out_channel*2, ker_size=1, padding=0, stride=stride,bn=bn)
...
class Discriminator(nn.Module):
    def __init__(self, opt):
        ...
        for i in range(opt.num_layer):
            ...
            block = Generator3DBlock(N,N)
            self.body.add_module('block%d' % (i), block)
        self.cb = ChannelAttention(out_planes=64)
        self.lb = SpatialAttention()
        # self.tail = nn.Sequential(ConvBlock(N, N, opt.ker_size, opt.padd_size, opt),ConvBlock(N, N, opt.ker_size, opt.padd_size, opt),nn.Conv3d(N, 1, kernel_size=opt.ker_size, padding=opt.padd_size))
